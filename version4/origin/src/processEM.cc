...
void ProcessEM::doEMEstimation(int iterationTimes)
{
    ...
    */
    ...
    {
        // set current interation 
        iterationTimes_Current_ = k;
        ...
        for(int j=0;j<sources_.size();j++)
        {
            ...
            // get current intensity for current source pixel
            //double intensityCurrent = sources_[sourceID].getIntensty();
            double intensityCurrent = sources_[sourceID].getIntensity(iterationTimes_Current_);
            cout<<"IterationID ; "<<k<<" ; sourceID ; "<<j<<" "<<" ; intensity ; "<<intensityCurrent<<endl;
        }

        // step 2 : output
        string name_part1 = to_string(k);
        string filename = "output_" + name_part1 + ".txt";
        ...
        for(int j=0;j<sources_.size();j++)
        {
            ...
            // get location for current pixel
            double locationx = sources_[sourceID].getX();
            double locationy = sources_[sourceID].getY();
            double locationz = sources_[sourceID].getZ();
            cout<<"sourceID ; "<<j<<" "<<" ; intensity ; "<<intensityCurrent<<" ; ";
            cout<<"x ; "<<locationx<<" ; y ; "<<locationy<<" ; z ; "<<locationz<<endl;
            write<<"sourceID ; "<<j<<" "<<" ; intensity ; "<<intensityCurrent<<" ; ";
            write<<"x ; "<<locationx<<" ; y ; "<<locationy<<" ; z ; "<<locationz<<endl;
        }
...
void ProcessEM::anIteration(int sourceID)
{
    ...
    double intensity_current = sources_[sourceID].getIntensity(iterationTimes_Current_);
    // s : probability that a photon emitted from pixel j would be detected anywhere, s_j
    ...
    */
    ...
    for(int i=0;i<detectors_.size();i++)
    {
        ...
        for(int k=0;k<sources_.size();k++)
        {
            ...
            double lambda_current = sources_[k].getIntensity(iterationTimes_Current_);
